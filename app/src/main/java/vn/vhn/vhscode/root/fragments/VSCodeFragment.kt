...
//        }
    ...
    private fun configureWebView(webView: WebView, force: Boolean = false) {
        ...
        webView.settings.fixedFontFamily = "termux-monospace"
                    ...
                    File(path).also { f ->
                        if (mSession?.isRemote == true) {
                            if (directoriesToOpen.isEmpty()) {
                                ...
                                filesToOpen.put("termux-remote://$host${f.absolutePath}")
                            }
                        } else if (f.exists()) {
                            if (f.isDirectory) directoriesToOpen.add(path)
                            else filesToOpen.put("termux-remote://$host${f.absolutePath}")
                        } else {
                            filesToOpen.put("termux-remote://$host${f.absolutePath}")
                        }
                    }
        ...
        webView.addJavascriptInterface(jsInterface!!, "_vn_vhn_termuxjs_")
        webView.loadUrl(url)
    }
