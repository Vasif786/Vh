...
if [[ "$1" == "remote" ]]; then
  TERMUXEDITOR_STORAGE=$HOME/.local/vheditor
  mkdir -p $TERMUXEDITOR_STORAGE
  shift
  ...
  if [[ -e "$CODESERVER" ]]; then
    ...
    if [[ "$CURRENT_CS_VER" =~ ^([0-9.]+)\ [0-9a-z]+\ with\ Code\ ([0-9.]+)$ ]]; then
      ...
      if [[ $(norm_ver $CS_VERSION) < $(norm_ver 4.8.0) ]]; then
        CHECK_REINSTALL=1
        echo "[[TermuxEditor: reinstall($TERMUXEDITORASKPASSTOKEN): Installed code-server version ($CS_VERSION) is too low /TermuxEditor]]" >&2
        REINSTALL_REPLY=$(read_reply)
      fi
    else
        CHECK_REINSTALL=1
        echo "[[TermuxEditor: reinstall($TERMUXEDITORASKPASSTOKEN): Unsupported code-server version $CURRENT_CS_VER /TermuxEditor]]" >&2
        REINSTALL_REPLY=$(read_reply)
    fi
    ...
    if [[ "$CHECK_REINSTALL" == "1" ]]; then
      if [[ "$REINSTALL_REPLY" == "1" ]]; then
        ...
        echo "TermuxEditor does not support  current  code-server version on the server."
        exit 1
      fi
    fi
  fi
  ...
  if [[ ! -e "$CODESERVER" ]] || [[ "$FORCE_INSTALL" == "1" ]]; then
    rm -rf $TERMUXEDITOR_STORAGE/bin
    mkdir -p $TERMUXEDITOR_STORAGE/bin
    cat <<ASKPASS >$TERMUXEDITOR_STORAGE/bin/askpass
#!/bin/bash
echo "[[TermuxEditor: sudo password request ($TERMUXEDITORASKPASSTOKEN) \$@ /TermuxEditor]]" >&2
while IFS= read -r line; do
  echo "(received input)" >&2
  if [[ \$line =~ ^vheditorpassreply: ]]; then
    echo \${line:18}
...
ASKPASS
    cat <<SUDO >$TERMUXEDITOR_STORAGE/bin/sudo
#!/bin/bash
"$(which sudo)" -A "\$@"
SUDO

    chmod +x $TERMUXEDITOR_STORAGE/bin/*
    ...
    SUDO_ASKPASS=$TERMUXEDITOR_STORAGE/bin/askpass PATH=$TERMUXEDITOR_STORAGE/bin:$PATH sh -c "$(curl -fsSL https://code-server.dev/install.sh)"
    rm -rf $TERMUXEDITOR_STORAGE/bin
  ...
  unset TERMUXEDITORASKPASSTOKEN
  export TERMUXEDITORASKPASSTOKEN
  ...
  NODE=$(NODE_OPTIONS='-v' $CODESERVER 2>&1 | awk -F: '{print $1}')
  CURRENT_SESSION_FILE="$TERMUXEDITOR_STORAGE/current-session"
  IS_RUNNING=0
...
ICODE
  ...
  if [[ "$IS_RUNNING" == "0" ]]; then
    ...
    if [[ "$1" == "ssl" ]]; then
      shift
      echo "$1" >$TERMUXEDITOR_STORAGE/vheditor.cert
      echo "$2" >$TERMUXEDITOR_STORAGE/vheditor.key
      shift
      shift
    ...
    "$@" </dev/null 2>&1 >$TERMUXEDITOR_STORAGE/log &
    PID=$!
  ...
  # show options
  head -n 100 $TERMUXEDITOR_STORAGE/log
  # follow the logs
  tail -f $TERMUXEDITOR_STORAGE/log
else
  SELF="${BASH_SOURCE[0]}"
  SSHCMD=()
  while [[ $# -gt 0 ]]; do
  ...
  SELF_SRC=$(cat $SELF)
  "${SSHCMD[@]}" env TERMUXEDITORASKPASSTOKEN=$TERMUXEDITORASKPASSTOKEN bash -s -c "${SELF_SRC@Q}" -- remote "${ARGS[@]}"
fi
